            ...
            <?php if(count($selectedtags)>0): ?>
                <div class="tag-container">
                    ...
                    <?php foreach ($selectedtags as $tag): ?>
                        <span class="badge badge-primary tag-item">
                                <?php echo $tag->name; ?>
                            <?php
                            if (key_exists('phpback_userid',$_SESSION)):
                                if ($_SESSION['phpback_userid'] == $idea->authorid || $_SESSION['phpback_isadmin']): ?>
                                    <a href="<?php echo base_url() . "action/removetag/" . $idea->id.'/'.$tag->id;?>" class="btn btn-sm btn-danger delete-tag" data-idea-id="<?php echo $idea->id; ?>" data-tag-id="<?php echo $tag->id; ?>">&times;</a>
                                <?php endif; endif; ?>
        </span>
                    <?php endforeach; ?>
                </div>
                ...
                <div>
                    ...
                    <ul style="list-style: none; padding-left: 0;">
                        <?php foreach ($attachments as $attachment): ?>
                            <li>
                                <a href="<?php echo base_url('public/uploads/' . $attachment->filename); ?>" target="_blank">
        ...
        <div class="row">
            <div class="col-md-10 col-md-offset-2">
                <ul class="nav-pills" style="list-style:none;margin-left:-40px;">
                    ...
                    <?php if ($_SESSION['phpback_userid'] == $idea->authorid): ?>
                        <li>
                            <div class="dropdown">
                                <button class="btn btn-info btn-sm dropdown-toggle" data-toggle="dropdown" style="width:130px"><?php echo 'Add Tag'; ?></button>
                                <span class="dropdown-arrow dropdown-arrow-inverse"></span>
                                <ul class="dropdown-menu dropdown-inverse">
                                    <?php foreach ($tags as $tag){ ?>
                                        <li><a href="<?php echo base_url() . "action/addtag/" . $idea->id.'/'.$tag->id;?>"><?php echo $tag->name; ?></a></li>
                                    <?php } ?>
                                </ul>
                            </div>
                        </li>
                    ...
                    <?php if ($_SESSION['phpback_userid'] == $idea->authorid): ?>
                        <li>
                            <div class="dropdown">
                                <button class="btn btn-info btn-sm dropdown-toggle" data-toggle="dropdown" style="width:130px"><?php echo $board_name; ?></button>
                                <span class="dropdown-arrow dropdown-arrow-inverse"></span>
                                <ul class="dropdown-menu dropdown-inverse">
                                    <?php foreach ($boards as $board){ ?>
                                        <li><a href="<?php echo base_url() . "action/changeBoard/" . $idea->id.'/'.$board->id;?>"><?php echo $board->name; ?></a></li>
                                    <?php } ?>
                                </ul>
                            </div>
                        </li>
